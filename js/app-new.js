...
(
	function ( document ) {
		...
		function registerEventListeners() {
			...
			for ( let k = 0; k < cardz.deck.length; k ++ ) {
				cardz.deck[ k ].addEventListener( 'click', function ( event ) {
		...
		function openCards( index ) {
			...
			if ( isMatch.count === 1 ) {
				...
				cardz.deck[ index ].classList.remove( 'flip' )
				cardz.deck[ index ].classList.add( 'open' )
				cardz.deck[ index ].classList.add( 'show' )
		...
		function getMatch( index ) {
			...
			let firstCard =
				cardz.deck[ matchTwoCards[ 0 ] ].children[ 0 ].getAttribute( 'data-class' )
			let secondCard =
				cardz.deck[ matchTwoCards[ 1 ] ].children[ 0 ].getAttribute( 'data-class' )

			cardz.deck[ index ].classList.add( 'open' )
			cardz.deck[ index ].classList.add( 'show' )

			console.log( 'length: ' + matchTwoCards.length )
			...
			if ( firstCard === secondCard && matchTwoCards[ 0 ] !== matchTwoCards[ 1 ] ) {
				...
				sleep( 5 ).then( () => {
					...
					window.setTimeout( () => {
						cardz.deck[ matchTwoCards[ 0 ] ].classList.add( 'mismatch' )
						cardz.deck[ matchTwoCards[ 1 ] ].classList.add( 'mismatch' )
					}, 350 )
				} )
				// need to pop the stack of both cards
				releaseCards()			}
		}
		...
		function lockCards() {
			...
			for ( let k = 0; k < 2; k ++ ) {
				cardz.deck[ matchTwoCards[ k ] ].classList.remove( 'open' )
				cardz.deck[ matchTwoCards[ k ] ].classList.remove( 'show' )
				cardz.deck[ matchTwoCards[ k ] ].classList.add(    'match' )
				updateScore()
		...
		function releaseCards() {
			...
			for ( let k = 0; k < matchTwoCards.length; k ++ ) {
				...
				sleep( 1000 ).then( () => {
					...
					//if ( 2 === cardz.deck.length ) {
						for ( let j = 0; j < matchTwoCards.length; j ++ ) {
							cardz.deck[ matchTwoCards[ k ] ].classList.remove( 'open' )
							cardz.deck[ matchTwoCards[ k ] ].classList.remove( 'show' )
							cardz.deck[ matchTwoCards[ k ] ].classList.remove( 'mismatch' )
						}
						matchTwoCards.pop()
						matchTwoCards.pop()
						isMatch.count = 0
